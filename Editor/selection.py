...
class Selection:
    ...
    def get_start_selection_y(self):
        return (
            self.start_selection_y
            if self.start_selection_y <= self.end_selection_y
            else self.end_selection_y
        )

    def get_start_selection_x(self):
        return (
            self.start_selection_x
            if (
                self.start_selection_y < self.end_selection_y
                or self.end_selection_x > self.start_selection_x
            )
        ...
        if self.container_y.is_empty():
            if self.container_x.is_empty():
                ...
                self.clipboard[self.clipboard_y] = (
                    text[cursor_y][cursor_x] + self.clipboard[self.clipboard_y]
                )
                self.container_x.add("Left")
    ...
    def up(self, stdscr, cursor_x: int, cursor_y: int, text: list[str]):
        if self.container_y.is_empty():
            ...
            stdscr.addstr(
                self.end_selection_y,
                0,
                text[self.end_selection_y][: self.end_selection_x],
                curses.A_REVERSE,
            )
            self.clipboard[0] = (
                text[self.end_selection_y][: self.end_selection_x] + self.clipboard[0]
            )
            ...
            self.clipboard[-1] = (
                ""
                if cursor_x == self.start_selection_x
                else (
                    text[cursor_y][self.start_selection_x : cursor_x]
                    if cursor_x > self.start_selection_x
                    else text[cursor_y][cursor_x : self.start_selection_x]
                )
            )
            self.clipboard_y -= 1
    ...
    def down(self, stdscr, cursor_x: int, cursor_y: int, text: list[str]):
        if self.container_y.is_empty():
            ...
            if not self.container_x.is_empty():
                self.clipboard[0] = text[self.start_selection_y][
                    self.start_selection_x :
                ]
